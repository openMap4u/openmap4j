...
import org.openmap4j.builder.style.TextStyleBuilder;
import org.openmap4j.style.TextStyleable;

public class TextStyleBuilderTest extends AbtractStyleBuilderTest {

	private TextStyleBuilder<TextStyleable> tsb = null;

	@BeforeEach
	public void beforeEach() {
		tsb = new TextStyleBuilder<TextStyleable>();
	}
	...
	public void testSetAll() {
		TextStyleable ts = tsb.alpha(ALPHA).visible(false).strokeCap(STROKE_CAP_VALUE).strokeColor(STROKE_COLOR)
				...
				.strokeMiterLimit(STROKE_MITER_LIMIT).strokeWidth(STROKE_WIDTH).fill(FILL).
				align(TEXT_ALIGN_VALUE).baseLine(FONT_BASE_LINE).fontFamily(FONT_FAMILY).fontSize(FONT_SIZE).fontStyle(FONT_STYLE_VALUE).
				build();
