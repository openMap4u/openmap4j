...
public abstract class AbstractLineTest extends CanvasTest<Shape> {
	...
	public void testWriteLine_DefaultLineStyle_no_params() {
		...
		Arrays.stream(MockData.X_COORDINATES).forEach(value -> canvas.draw(getGeometry(value, y)));
	}

	@Test
	@DisplayName("alpha style")
	public void testWrite_alphaStyle_param() {
		double y = 150;
		canvas.draw("Default style alpha param", 5, y - 20);
		for (int i = 0; i < 10; i++) {
			canvas.draw(getGeometry(i * 100 + 50, y),
	...
	public void testWrite_strokeDashOffset_param() {
		...
		for (int i = 0; i < 10; i++) {
			canvas.draw(getGeometry(i * 100 + 50, y), StyleBuilderFactory.getLineStyleBuilder().strokeWidth(20)
	...
	protected Writer getWriter() throws IOException {
		return super.getFile("LineTest.svg");
	}
	...
	Shape getGeometry(double x, double y) {
		return GeomBuilderFactory.getLineBuilder().moveTo(x - 40, y).lineTo(x, y + 40).lineTo(x + 40, y - 40).build();
	}
